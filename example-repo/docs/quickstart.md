...
`xo serve` runs the regular web UI backed by SQLite sync storage. TUI clients configure `.exo/tui.toml`, run `start-sync` once, and then continue automatically after approval in the web UI's `sync clients` tab.
